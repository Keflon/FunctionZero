...
namespace AnimationZero.Clock
{
    public class ClockEventManager
    {
        ...
        // Manages whether there is anything to update, ie.e whether the external clock needs to run.
        ...
        public void Tick(int clock)
        {
            if (!_allowReentrancy)
            {
                if (_reentrant)
                {
                    throw new InvalidOperationException("Timer is not configured for re-entrancy and a call is in progress in ClockEventManager.Tick()");
                }
                _reentrant = true;
            }
